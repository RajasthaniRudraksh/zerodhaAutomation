import socket,datetime,threading,time,json,pickle,configparser
...
# logName=f'log {datetime.datetime.now().strftime("%d-%m-%Y %H:%M:%S")}.log'
...
if today!=d.date():
    ...
    instrument=location.joinpath('instrument.txt')
    file=open(instrument,'w')
    print('Saving Instrument Data...')
    with open(instrument,'wb') as fp:
        pickle.dump(data,fp)
    with open(location.joinpath('config.ini'),'w') as fb:
        ...
        config.write(fb)
    print("Instrument File Downloaded.")
...
def updateOrderUnit(order):
    ...
    if statusX not in ['COMPLETE','REJECTED','CANCELLED']:
        if status!='UPDATE':
            ...
            ORDERS[orderId]['msg']=order.get('status_message')
...
def orderStatusRequest(request):
    ...
    if request.get('orderId')==None:
        ...
        return 0    
    try:
        orderU=ORDERS[request.get('orderId')]
        status=orderU.get('orderStatus')
        response={'processId':request.get('processId',-1) ,'status':1,'orderStatus':status,'order':orderU}
    except:
...
def orderPlaceHigh(request):
    ...
    # if orderResult.get('status')==1:
    #     temp={}
    #     temp['ip']=request.get('ip')
    #     temp['orderStatus']='OPEN'
    #     orderLock.acquire()
    #     ORDERS[orderResult.get('orderId')]=temp.copy()
    #     orderLock.release()       
...
def modifyOrderService(request):
    ...
    try:
        ...
        if e.find('Maximum allowed order modifications exceeded')!=-1:
            ...
            if resultX.get('status')==1:
                response={'processId':request.get('processId',-1) ,'status':1,'msg':f'Cancelled & Recreated Successfully {e}'}
            else:
...
def cancelOrderService(request):
